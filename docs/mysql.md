...
- LDFLAGS 是一个在软件编译构建过程中使用的环境变量或 Makefile 变量，全称为 Linker Flags（链接器标志）
  > - 指定库文件搜索路径: 最常用的选项是 -L。例如 LDFLAGS = -L/usr/local/lib 会告诉链接器在标准路径之外，也要去该目录寻找需要的库文件
  > - 运行时库搜索路径：通过 -Wl,-rpath 等参数，可以指定程序在运行时去哪里加载动态库。
  > - 优化与调试控制：可以包含链接阶段的优化参数，或者去除符号表以减小文件体积（如 Go 语言中的 -s -w）。
