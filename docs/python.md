...
- `__doc__`：定义文档字符串
- `__dict__`：类的属性列表
- `__class__`：返回当前类对应的变量
- `__slots__`：在运行时对类的实例动态绑定属性和方法
- `__name__`：返回当前类的类名，默认是当前模块名（）文件名；如果当前文件被当做脚本直接执行，值为\_\_main\_\_
...
>
> - 类体中、所有函数之外：此范围定义的变量，称为`类属性或类变量（class variables）`
